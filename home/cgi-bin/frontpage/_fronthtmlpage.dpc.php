...
class fronthtmlpage {
	...
	static $staticpath;
	var $BASE_URL, $MC_ROOT, $MC_TEMPLATE, $MC_DEBUG;
...
EOF;
 ...
 /* function tokenizer($tokarray) {
  ...
  function detokenizer($tok) {
	...
	public function mcSelectPage($id=null,$defpage=null,$tmplname=null) {
	    $db = GetGlobal('db');
		...
		if ($this->MC_TEMPLATE) {
			...
			if ($tmplname) 
			  $sSQL.= 'and mctmpl='. '"' . $tmplname . '" ';
			else
              $sSQL.= 'and mctmpl='. '"' . $this->MC_TEMPLATE . '" ';
            //echo $sSQL;			  
